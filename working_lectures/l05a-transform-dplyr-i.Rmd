---
title: "BST430 Lecture 05-A"
subtitle: "Data transformation with dplyr"
author: "Seong-Hwan Jun, based on the course by Andrew McDavid and Tanzy Love"
...
- HW1 assigned, due next Wednesday 5pm.
- Reminder: lab 2 due at 5pm. 
- TA office hours are posted on class website.
  + Hanwen: Thursday 1 - 2 pm.
  + Abby: 12:15 - 1:15 pm.
- I don't have office hour but email me if you need to talk to me.
...
<!-- <sup>1</sup> -->
.pull-left[
![Magritte's representation of a pipe (that's not a pipe!)](l05/img/magritte.jpg)
]
.pull-right[
![magrittr](l05/img/magrittr.jpg)
]
...
tanzy %>% find(what = "helmet") %>%
  ...
  lock()
```
.footnote[[1] Note similar syntax in java(script), C++ and python where the method accessor "`.`" can be chained in some cases: `tanzy.find('helmet').ride('campus').lock()`]
...
- Shortcut for piping: cmd/ctrl + shift + M.
- Since R 4.1, there is also a native pipe operator `|>` in base R. Enable this `Tools-> Global Options -> Code -> Use R native pipe operator`. 
- It is always available even when you are not using `tidyverse`.

---

## Useful tips

- `|>` behaves similarly, but not identically to magrittr's `%>%`.
  + To send results to a function argument other than first one or to use the previous result for multiple arguments, use `_` instead of `.`.

```{r}
starwars |> lm(mass ~ height, data = _) 
```
...
Here's the [R code in this lecture](https://github.com/seonghwanjun/bst430-fall2025-l05/blob/main/transform-dplyr-i.R)
...
* <s>`transmute`: create new data frame with variables</s> superseded by `mutate(.keep="none")`
* `distinct`: filter for unique rows
* <s>`sample_n` / `sample_frac`: randomly sample rows</s> superseded by `slice_sample()`
...
.footnote[[1] Or `tibble`, the tidyverse version of a `data.frame`.]
---
...
Using the `flights` data frame from the `nycflights13` package, write code to do the following tasks:

1. .question[Extract the January LGA flights and the December JFK flights.]
2. .question[Extract flights whose `dep_time` was `NA`.]
3. .question[Extract flights that left in odd months (i.e., 1, 3, 5, 7, 9, 11).]
...
```
flights %>% filter(month %in% seq(1, 11, 2))
```
...
- **Exercise**: `dep_time` is convenient to look at, but hard to compute with because it's not a continuous value:
...
This function has been superseded now by `mutate(.keep="none")`.
...
.footnote[[1] These probably only make sense in `group_by` summaries.  They could make sense in an ungrouped mutates, though!]
--
...
## Subsample rows with `slice_sample`

`sample_n` and `sample_frac` are soft-deprecated.  Use `slice_sample` instead.
...
`relocate` moves column to the front -- can pass in `.after` or `.before` argument to specify location to place column(s).
...
.pull-left[
![pipe](l05/img/hex-dplyr.png)
]
.pull-right[
![magrittr](l05/img/Octocat.png)
]
...
How might we calculate the carrier with the worst on-time percentage?
...
By adding `group_by()` to a pipeline, we stratify by a categorical variable when we `summarize()`:
.pull-left[
**Grand margin**
```{r, warning=FALSE}
library(nycflights13)
...
Default: Drop groups formed by factor levels that don't appear in the data. `.drop=FALSE` is needed to preserve all group levels.

.footnote[[1] We'll discuss the pain and pleasure of factors in greater detail.]
...
By the way, `summarize` seems to group the output only by `carrier`? what about `hour`?

```{r}
departures = flights %>% group_by(carrier, hour) %>% summarize(n_departures = n())
```

When you call `summarize` it requires computing a summary over a group of values -- by default, `summarize` achieves this by dropping the last group (`hour` in this case). 
...
We'll discuss the `fct_reorder` business in the coming weeks.
...
`select()` (as well as `pivot_wider()`, `pivot_longer()`, and friends) use [tidy-select](https://dplyr.tidyverse.org/reference/dplyr_tidy_select.html), which is a meta-language to select columns from a data frame. There are several helper functions you can use within `select()`:
...
The syntax is a bit idiomatic, but in general, it looks something like
```r
...
- [Chapter 3 R4DS](https://r4ds.hadley.nz/data-transform.html)
- [Chapter 12 R4DS](https://r4ds.hadley.nz/logicals.html)
- [Chapter 13 R4DS](https://r4ds.hadley.nz/numbers.html)
