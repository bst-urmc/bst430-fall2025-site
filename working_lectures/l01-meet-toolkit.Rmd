---
title: "BST430  Lecture 01"
subtitle: "Meet the toolkit"
author: "Seong-Hwan Jun, based on the notes of Andrew McDavid and Tanzy Love"
...
-  Making it easier to reproduce analyses may reduce the temptation to commit fraud.
...
- Literate programming (code, narrative, output in one place) $\rightarrow$ R Markdown / Quarto.
...
- Tips for selecting a username as per [HGWR](http://happygitwithr.com/github-acct.html#username-advice):
...
- Once you have a github account, click the link to join the github classroom (emailed). 
...
- There are hundreds of git commands -- it's used for the Linux kernel for chrissakes.  But 95% of the time you will only need three commands: `commit`, `push`, `pull` (maybe also `add`  and adjust the `remote` repositories you sync with).
- We will be doing Git things and interfacing with GitHub through RStudio but I recommend either installing a GUI-based software client
  e.g., [GitHub Desktop](https://desktop.github.com/), [Sourcetree](https://www.sourcetreeapp.com/), or [GitKraken](https://www.gitkraken.com/)) or learning the command line interface (CLI) [GitHub CLI](https://cli.github.com/).
...
- Run the analysis using **Quarto**

- Edit the file

- Re-run the analysis using **Quarto**
...
- Quarto is a new, modern, and powerful tool for creating dynamic documents, presentations, and reports.
- It is the successor to R Markdown, and is fully compatible with R Markdown documents.
- Quarto works not only with R, but also with Python, Julia, and other languages.
