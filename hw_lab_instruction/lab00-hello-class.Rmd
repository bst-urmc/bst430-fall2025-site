...
```{r hotdogs}
hotdogs=read.table("lab00-demonstration/files/Hdcalsod.TXT", header=T)
...
legend("upperleft", levels(factor(Type)), col=1:3, pch=1)
```

Let's try a different data set. This one about 50 states.

```{r raingrad}
hsgrads=read.table("lab00-demonstration/files/raingrad.TXT", header=T, sep="\t")
attach(hsgrads)
...
```{r delta}
delta=read.table("lab00-demonstration/files/Deltafly.TXT", header=T)
attach(delta)
